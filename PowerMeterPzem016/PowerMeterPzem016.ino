...
| |_) / _ \ \ /\ / / _ \ '__| | |\/| |/ _ \ __/ _ \ '__|      Daniele Facco
|  __/ (_) \ V  V /  __/ |    | |  | |  __/ ||  __/ |   
|_|   \___/ \_/\_/ \___|_|    |_|  |_|\___|\__\___|_|         FarSideVirtual@airmail.cc
*/
#include <WiFi.h>
...
void loop() {
  ...
  for (pzemSlaveAddr = 1; pzemSlaveAddr < 2; pzemSlaveAddr++) {  // Loop all the Pzem sensors
    ...
    if (result == node.ku8MBSuccess)
    {
      ...
      Serial.print("V   ");
      String v0 = (String) voltage;
      char message_buff0[5];
      v0.toCharArray(message_buff0, v0.length()+1); 
      client.publish("cmnd/test321/v", message_buff0); 
      
      Serial.print(hz, 1);
      Serial.print("Hz   ");
      String hz0 = (String) hz;
      char message_buff1[5];
      hz0.toCharArray(message_buff1, hz0.length()+1); 
      client.publish("cmnd/test321/hz", message_buff1);

      Serial.print(current, 3);
      Serial.print("A   ");
      String a0 = (String) current;
      char message_buff2[5];
      a0.toCharArray(message_buff2, a0.length()+1); 
      client.publish("cmnd/test321/a", message_buff2);

      Serial.print(power, 1);
      Serial.print("W  ");
      String w0 = (String) power;
      char message_buff3[5];
      w0.toCharArray(message_buff3, w0.length()+1); 
      client.publish("cmnd/test321/w", message_buff3);

      Serial.print(pf, 2);
      Serial.print("pf   ");
      String pf0 = (String) pf;
      char message_buff4[5];
      pf0.toCharArray(message_buff4, pf0.length()+1); 
      client.publish("cmnd/test321/pf", message_buff4);

      Serial.print(energy, 0);
      Serial.print("Wh  ");
      String wh0 = (String) energy;
      char message_buff5[5];
      wh0.toCharArray(message_buff5, wh0.length()+1); 
      client.publish("cmnd/test321/wh", message_buff5);
